...
for element in data:
    ...
    robot = {"name": robot_data[0], "processing_time": int(robot_data[1]), "free_at": time}
    robots.append(robot)
...
while len(products) > 0:
    ...
    if available_robots:
        ...
        current_robot['free_at'] = time + timedelta(seconds=current_robot['processing_time'])
        robot = [el for el in robots if el == current_robot][0]
        robot['free_at'] = time + timedelta(seconds=current_robot['processing_time'])
